...
function Itens() {
  ...
  const headers = [
    ...
    { label: "Quantidade Promocional", key: "promoQuantity" },
    { label: "Preço de Atacado", key: "promoValue" },
  ];

  const csvData = tabela?.products;
  console.log("csv", csvData);

  const csvReport = {
    data: csvData,
    headers: headers,
    filename: "ListaDeCompras.csv",
  };
  ...
  const getTabelaData = async () => {
    try {
      let data = await api
        .get(/shopcarts/ + localStorage.getItem("carId"), {
  ...
  const criarItem = async () => {
    try {
      await api
        .post(
          "/products",
          { name: nome, category_id: chosenCat, price: value },
          { headers: { Authorization: localStorage.getItem("myKey") } }
        )
        .then(() => {
          getProdutos();
    ...
    try {
      await api
        .post(
          "/products_shopcarts",
          {
            shopcart_id: localStorage.getItem("carId"),
            product_id: producId,
            promo_quantity: quantAtacado,
      ...
      <body className="body">
        <div class="panel-body table-responsive">
          ...
          <Table striped bordered hover className="table">
            ...
            <tbody className="corpo-lista">
              {tabela.products &&
                tabela.products.map((item) => (
                  <tr key={`product${item.id}`}>
                    <td>{item.id}</td>
                    <td>{item.name}</td>
                    <td>{item.quantity}</td>
                    <td>
                      {item.quantity > item.promoQuantity &&
                      item.promoQuantity != null
                        ? item.promoValue
                        : item.price}
                    </td>
                    <td>
                    ...
                    <td>
                      ...
                      <button
                        ...
                        onClick={(e) => deleteIten(item.productsShopcartId)}
                      >
                        {" "}
                        <i class="far fa-trash-alt">Deletar</i>
                      </button>
                    </td>
                  </tr>
                ))}
        ...
        <div>
          <form className="adicionar-itens">
            ...
            <div className="div-itens" id="Nome">
              <p> Nome do Item </p>
            </div>{" "}
            <div>
              {" "}
          ...
          <form>
            ...
            <div>
              ...
              <input
                className="inputs"
                type="text"
                onChange={(e) => setQuantatacado(e.target.value)}
              />
            </div>
            <div className="div-itens">
              <p> Valor de Atacado </p>
            </div>{" "}
            <div>
              {" "}
              <input
            ...
            <div>
              ...
              <input
                ...
                type="submit"
                value="Adicionar"
                onClick={(e) => addCarrinho()}
                disabled={!quantAtacado || !promoValue || !quant || !producId}
              />
            </div>
          </form>
        </div>
